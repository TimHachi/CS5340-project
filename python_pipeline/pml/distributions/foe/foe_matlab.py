...
#     %  relevant data from OTHER.
    def __init__(self):
        if (nargin == 1):
            ...
            self.experts = {self.default_expert};
    ...
    def set_imdims();
        if any(this.imdims ~= imdims)
            if numel(imdims) ~= 2 || any(imdims(:) <= 0)
                raise ValueError('IMDIMS must be a positive 2-element vector.')
            if any(imdims < this.filter_size)
                raise ValueError('IMDIMS < (max) FILTER_SIZE')
...
#       experts = experts(:)';
    def set_experts(experts)
        if (not iscell(experts)) || (len(experts) == 0):
            raise ValueError('Experts amount must be more than 1')
...
#         case 'same'
#           this.conv2 = @(img, f) conv2(img, f, 'same');
#           this.imfilter = @(img, f) imfilter(img, f, 'same');
#           this.makemat = @(this, f) pml.image_proc.make_convn_mat(f, this.imdims, 'same');
